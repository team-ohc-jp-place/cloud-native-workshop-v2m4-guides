...
起動に *9ミリ秒* かかりました。お使いの環境によっては起動時間が異なる場合があります。
...
----
    PID   RSS COMMAND
   1020 68960 /tmp/hello/getting-started/target/getting-started-1.0-SNAPSHOT-runner
----

これは、私たちのプロセスが 67MB 程度のメモリを消費していることを示しています (https://en.wikipedia.org/wiki/Resident_set_size[Resident Set Size^] 、または RSS)。かなりコンパクトです。
