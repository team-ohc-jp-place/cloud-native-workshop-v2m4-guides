...
=== 3. カートサービスに Kafka クライアントを追加

これまでに、小売店のショッピングデータで動作するマイクロサービスをいくつか追加しました。例えば、一度ユーザーがチェックアウトすると、注文サービスや支払いサービスのような他のサービスがこの情報を必要とし、さらに処理をしたいと考えています。そこで、カートサービスを Kafka と統合して、買い物客がチェックアウトしたときに注文メッセージを送信できるようにします。
...
これにより、カートサービスアプリに Kafka 拡張機能と API が追加されます。

支払いサービスと同様に、このコードを `// TODO: ワークスペースの `cart-service` サブディレクトリの `com.redhat.cloudnative` パッケージ内の `CartResource` クラス内の `ここに注文メッセージング設定の注釈を追加する` マーカーに追加します :
